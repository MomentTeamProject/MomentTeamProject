...
public class CommonService {
	...
	@Autowired private WebUserServiceImpl mailservice;
	...
	public String json_list(StringBuilder url) {
		...
		try {
			json = (JSONObject)new JSONParser().parse(data_list(url));
			json = (JSONObject)json.get("response");
			json = (JSONObject)json.get("body");
			//body내에 items, pageNo, totalCount
	...
	public File download(String filename, String filepath
		...
		File file = new File(
				session.getServletContext().getRealPath("resources") + filepath );
...
public void sendHtml(WebUserVO vo, HttpSession session) throws Exception {
	//private void sendHtml(String email, String name, HttpServletRequest request) {
...
//			msg.append("<a href='http://hanuledu.co.kr'><img src='http://hanuledu.co.kr/data/menu/LOGO_YOlQQBRgVrzJC2PUyqB31583994097.jpg'/></a>");
			...
			msg.append( // 메일내용
					"<h1>메일인증</h1>" +
					"<a href='http://localhost/moment/user/useremailConfirm?userid=" + vo.getU_userid() +
					"&key=" + key +
					"' target='_blenk'>이메일 인증 확인</a>");
//			msg.append("<a href='http://localhost:80"+ request.getContextPath()+user'>인증하기</a>");
			msg.append("</body>");
			msg.append("</html>");
